...
async function runMeta(prompt: string, model: string): Promise<ProviderRunOutput> {
  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey) {
    throw new Error("OPENROUTER_API_KEY não configurada para modelos Meta");
  }
  ...
  if (!response.ok) {
    throw new Error(`Meta via OpenRouter falhou: ${await response.text()}`);
  }
...
export const metaProvider: LlmProvider = {
  name: "meta",
  models: ["meta-llama/llama-3.3-70b-instruct", "meta-llama/llama-3.1-405b-instruct", "meta-llama/llama-3.1-70b-instruct", "meta-llama/llama-3.1-8b-instruct", "meta-llama/llama-3.2-90b-vision-instruct"],
  run: runMeta
};
