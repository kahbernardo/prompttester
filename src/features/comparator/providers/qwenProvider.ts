...
async function runQwen(prompt: string, model: string): Promise<ProviderRunOutput> {
  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey) {
    throw new Error("OPENROUTER_API_KEY não configurada para modelos Qwen");
  }
  ...
  if (!response.ok) {
    throw new Error(`Qwen via OpenRouter falhou: ${await response.text()}`);
  }
...
export const qwenProvider: LlmProvider = {
  name: "qwen",
  models: ["qwen3.6-plus", "qwen3.6-plus-256k"],
  run: runQwen
};
