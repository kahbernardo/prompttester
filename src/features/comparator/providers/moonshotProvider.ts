...
async function runMoonshot(prompt: string, model: string): Promise<ProviderRunOutput> {
  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey) {
    throw new Error("OPENROUTER_API_KEY não configurada para modelos Moonshot");
  }
  ...
  if (!response.ok) {
    throw new Error(`Moonshot via OpenRouter falhou: ${await response.text()}`);
  }
...
export const moonshotProvider: LlmProvider = {
  name: "moonshot",
  models: ["kimi-k2-0905-preview", "kimi-k2-0711-preview", "kimi-k2-turbo-preview", "kimi-k2-thinking", "kimi-k2-thinking-turbo"],
  run: runMoonshot
};
