...
async function runMistral(prompt: string, model: string): Promise<ProviderRunOutput> {
  ...
  if (!response.ok) {
    throw new Error(`Mistral falhou: ${await response.text()}`);
  }
...
export const mistralProvider: LlmProvider = {
  name: "mistral",
  models: ["mistral-large-latest", "mistral-small-latest", "ministral-8b-latest", "codestral-latest", "pixtral-large-latest"],
  run: runMistral
};
